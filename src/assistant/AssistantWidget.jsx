...
const T = {
  en: { title: 'Rent Pondy AI Assistant', placeholder: 'Ask about rentals…', hi: "👋 Welcome! I'm your Rent Pondy assistant. I can help you find rentals, check your points and more.", micHint: 'To talk to me, just tap the microphone button.', listen: 'Listening… tap to stop', tapPlay: '🔊 Tap to play reply', details: 'Details', confirm: 'Confirm', addTenant: '➕ Add Tenant Assistance', addProperty: '🏠 Add Property', sent: 'Done ✓', spend: 'Spends points', voiceListen: '🎙️ Listening…', voiceThink: '💭 Thinking…', voiceSpeak: '🔊 Speaking…', callStart: 'Hands-free voice', callEnd: 'End voice', pressToChat: 'AI Assistant', callHint: 'Tap to talk with me', voicePrompt: 'How can I help you?' },
  ta: { title: 'ரெண்ட் பாண்டி உதவியாளர்', placeholder: 'வாடகை பற்றி கேளுங்கள்…', hi: '👋 வணக்கம்! நான் உங்க Rent Pondy assistant. வீடு தேட, points பார்க்க எல்லாம் help பண்ணுவேன்.', micHint: 'என்னோட பேச, மைக் பட்டனை அழுத்துங்க.', listen: 'கேட்கிறேன்… நிறுத்த தட்டவும்', tapPlay: '🔊 பதிலைக் கேட்க தட்டவும்', details: 'விவரம்', confirm: 'உறுதி', addTenant: '➕ Tenant Assistance சேர்க்க', addProperty: '🏠 Property போடு', sent: 'முடிந்தது ✓', spend: 'புள்ளிகள் செலவாகும்', voiceListen: '🎙️ கேட்கிறேன்…', voiceThink: '💭 யோசிக்கிறேன்…', voiceSpeak: '🔊 பேசுறேன்…', callStart: 'குரல் மோட்', callEnd: 'நிறுத்து', pressToChat: 'AI உதவியாளர்', callHint: 'பேச தட்டவும்', voicePrompt: 'சொல்லுங்க, நான் எப்படி உதவட்டும்?' },
};
    ...
    <>
      {!open && (
        <button className="rp-fab" aria-label="Open AI assistant" onClick={() => { voice.unlockAudio(); setOpen(true); }}>
          <span className="rp-fab-ping" aria-hidden="true" />
          <svg className="rp-fab-icon" width="30" height="30" viewBox="0 0 24 24" fill="none" aria-hidden="true">
            <path d="M6.5 4h11A3.5 3.5 0 0 1 21 7.5v5A3.5 3.5 0 0 1 17.5 16H12l-4.15 3.46A.6.6 0 0 1 6.8 19V16h-.3A3.5 3.5 0 0 1 3 12.5v-5A3.5 3.5 0 0 1 6.5 4Z" fill="#fff" />
            <path d="M10.6 6.2c.36 2.65 1.18 3.47 3.83 3.83-2.65.36-3.47 1.18-3.83 3.83-.36-2.65-1.18-3.47-3.83-3.83 2.65-.36 3.47-1.18 3.83-3.83Z" fill="#0e9f6e" />
            <path d="M16.1 5.4c.16 1.18.53 1.55 1.71 1.71-1.18.16-1.55.53-1.71 1.71-.16-1.18-.53-1.55-1.71-1.71 1.18-.16 1.55-.53 1.71-1.71Z" fill="#0e9f6e" />
          </svg>
        </button>
      )}
        ...
        <div className="rp-panel" role="dialog" aria-label={t.title}>
          <div className="rp-head">
            ...
            <div className="rp-head-tools">
              ...
              <button className="rp-ic" title="Close" onClick={() => setOpen(false)}>✕</button>
            </div>
          </div>
              ...
              <div key={m.id} className={`rp-msg rp-${m.role}`}>
                ...
                {m.cards && m.cards.length > 0 && (
                  <div className="rp-cards">
                    {m.cards.map((c, i) => (
                      <ResultCard key={`${m.id}-c${i}`} card={c} t={t} onOpen={(rentId) => navigate(`/detail/${rentId}`)} />
                    ))}
                  </div>
                )}
          ...
          {voiceMode && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', background: '#0e9f6e', color: '#fff', fontSize: 13, fontWeight: 600 }}>
              <span style={{ width: 9, height: 9, borderRadius: '50%', background: '#fff', opacity: voiceStatus === 'listening' ? 1 : 0.5 }} />
              <span>{voiceStatus === 'speaking' ? t.voiceSpeak : voiceStatus === 'thinking' ? t.voiceThink : t.voiceListen}</span>
              <button onClick={stopVoiceMode} style={{ marginLeft: 'auto', background: 'rgba(255,255,255,.25)', border: 'none', color: '#fff', borderRadius: 12, padding: '3px 12px', cursor: 'pointer', fontWeight: 700 }}>{t.callEnd} ✕</button>
            </div>
          )}
          ...
          <div className="rp-input">
            ...
            <input
              ...
              disabled={busy}
            />
            <button className="rp-send" onClick={() => submitText(input)} disabled={busy || !input.trim()}>➤</button>
          </div>
        </div>
      )}
    </>
